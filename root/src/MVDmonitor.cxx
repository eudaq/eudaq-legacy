...
void Variable(){
	...
	//Int_t ShiftX[glob->TELNumberOfPlanes];
	//Int_t ShiftY[glob->TELNumberOfPlanes];
...
Int_t FindEvent(Int_t EndEvent){
	...
	while (1) {
		...
		while (1) {
			...
			temp_bit[k] = tmp_bit;
			//printf("i=%d, tmp= 0x%x \n", k, temp_bit[k]);
			//search label for number events
			if (k > 4) {
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		while (1) {
			...
			while (1) {
				...
				//printf("i=%d, tmp= 0x%x \n", k, temp_bit[k]);
				if (k > 4) {
					if ((temp_bit[k - 3] == 0xAA) & (temp_bit[k - 2] == 0xBB) & (temp_bit[k - 1] == 0xCC) & (temp_bit[k] == 0xD0))  break;
				}//if
				k++;
			}//while
			TelPointer = 0;
			...
			//FIXME
			for (Int_t k = 0; k < ADCXSize; k++) 		SeedX[k] = false;
			for (Int_t k = 0; k < ADCXSize; k++) 		SeedY[k] = false;
...
while(1){
	...
	for(Int_t i = 1; i<2000; i++ ){
		if (all_event > i * 2000) start_event =i * 2000;
		if (all_event + 2000 < all_event ) break;
	}
