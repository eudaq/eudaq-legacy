...
namespace eudaq {
  ...
  private:
	...
	for (size_t Row = 0; Row < m_NumRows; ++Row) {
	  ...
	  std::cout << "Row = " << Row << " Header = " << std::hex << getlittleendian<unsigned short>(&data[header_offset]) << "    " << getlittleendian<unsigned short>(&data[header_offset+sizeof (short) ]) << std::endl ;
