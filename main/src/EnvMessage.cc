...
namespace eudaq {
  ...
  EnvMessage::EnvMessage(const std::string & msg, Level level, const Time & time)
    : Status(LVL_ENV, msg), m_line(0), m_time(time), m_createtime(Time::Current())
  {}

  EnvMessage::EnvMessage(Deserializer & ds)
  ...
  EnvMessage EnvMessage::Read(std::istream & is) {
...
//        << m_func << "\n";
    ...
    if (parts.size() != 6) EUDAQ_THROWX(FileFormatException, "Badly formatted line");
      ...
      return eudaq::EnvMessage(parts[1], (eudaq::Status::Level)level, time)
  ...
  void EnvMessage::Serialize(Serializer & ser) const {
  ...
  void EnvMessage::Print(std::ostream & os) const {
    os << Level2String(LVL_ENV) << ": " << m_msg
       << " " << m_time.Formatted();
  ...
  void EnvMessage::Write(std::ostream & os) const {
    os << Level2String(LVL_ENV) << "\t"
       << escape_string(m_msg) << "\t"
       << m_time.Formatted() << "\t"
       << GetSender() << "\t"
       << m_file << ":" << m_line << "\t"
  ...
  EnvMessage & EnvMessage::SetLocation(const std::string & file, unsigned line, const std::string & func) {
    m_file = file;
    m_line = line;
    m_func = func;
    return *this;
  ...
  EnvMessage & EnvMessage::SetSender(const std::string & name) {
    size_t i = name.find('.');
