...
bool DoEvent(unsigned ndata, eudaq::DetectorEvent * dev, bool do_process, bool do_display, bool do_zs, bool do_dump,eudaq::EUDRBDecoder * decoder, std::ofstream * mimoromafile) {
  ...
  if (do_process || do_display || do_dump || do_zs) {
    ...
    for (size_t i = 0; i < dev->NumEvents(); ++i) {
      ...
      eudaq::RawBytesEvent * rawEvent = dynamic_cast< eudaq::RawBytesEvent*> ( subev );
      if ( rawEvent ) {
        mimoromafile->write( reinterpret_cast<char *> ( rawEvent->GetData()) , rawEvent->DataSize());
      }
    }
  }
  return do_display;
...
int main(int /*argc*/, char ** argv) {
  ...
  try {
    ...
    for (size_t i = 0; i < op.NumArgs(); ++i) {
      ...
      std::string mimoromafile ;
      if (datafile.find_first_not_of("0123456789") == std::string::npos) {
        int runNumber = from_string(datafile, 0);
        datafile = "../data/run" + to_string(from_string(datafile, 0), 6) + ".raw";
        mimoromafile = "../data/run" + to_string( runNumber, 6 ) + "-mimoroma.raw";
      }
      
      std::ofstream * file = new std::ofstream( mimoromafile.c_str(), std::ios::binary | std::ios::out );
      ...
      while (des.HasData()) {
        ...
        if (ev->IsBORE()) {
          ...
          bool shown = DoEvent(ndata, dev, proc, show, do_zs.IsSet(), dump, decoder.get(), file);
          if (showlast) {
            if (shown) {
              lastevent = 0;
            } else {
      ...
      if (lastevent.get()) DoEvent(ndatalast, dynamic_cast<eudaq::DetectorEvent*>(lastevent.get()), false, true, do_zs.IsSet(), false, decoder.get(), file);
      file->close();
