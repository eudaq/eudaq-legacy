...
protected:
	...
	std::vector<SimpleStandardHit> _section_hits[4];//FIXMWE hard-coded
	std::vector<SimpleStandardCluster> _section_clusters[4];
public:
	...
	void addHit(SimpleStandardHit oneHit) {//oneHit.reduce(_binsX, _binsY); //also fills the appropriate sections
		_hits.push_back(oneHit);
		if  (_name.compare("MIMOSA26")==0)
		{
			unsigned int section =oneHit.getX()/288;
