...
const unsigned int mimosa26_max_section=4; //number of sections in MIMOSA26
...
protected:
	...
	TH1I * _clusterXWidth;
	TH1I * _clusterYWidth;
	TH1I * _nHits_section[4];
	TH1I * _nClusters_section[4];
...
public:
		...
		if (_maxX != -1 && _maxY != -1) {
			...
			_hitOcc= new TH1F(out2, out,500,0.01, 1);
			...
			for (int section=0; section<4; section++)
			{
				...
				_nHits_section[section]= new TH1I(out2, out,100,0,100);
				...
				_nClusters_section[section]= new TH1I(out2, out,100,0,100);
	...
	*/
	...
	void Fill(const SimpleStandardPlane plane) {
		...
		if (plane.getName().compare("MIMOSA26")==0)
		{
			for (int section=0; section<4; section++)
			{
		...
		*/
		...
		for (int x = 0; x < _maxX; ++x) {
			for (int y = 0; y < _maxY; ++y) {
				double bin = _hitmap->GetBinContent(x,y); //FIXME
